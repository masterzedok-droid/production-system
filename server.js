...
// Подключение к базе данных (ЗАМЕНИТЕ ПАРОЛЬ!)
const pool = new Pool({
    host: 'localhost',
    port: 5432,
    database: 'production',
    user: 'postgres',
    password: '007cyber007'  // ← ВАШ ПАРОЛЬ (который задали при установке)
});
...
app.put('/api/order/:number/complete', async (req, res) => {
    await pool.query(
        `UPDATE orders SET status = 'completed' WHERE number = $1`,
        [req.params.number]
    );
    res.json({ success: true });
});

// ===== ВЕБ-СТРАНИЦА (полный HTML из дня 4) =====
app.get('/', (req, res) => {
    res.send(`
        <!DOCTYPE html>
        <html>
...
// Запуск сервера
app.listen(3000, () => {
    console.log('🚀 Сервер с БД запущен на http://localhost:3000');
});